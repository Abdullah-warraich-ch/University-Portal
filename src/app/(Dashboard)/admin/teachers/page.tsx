...
import { Plus, Edit2, Trash2, Search, Filter, Briefcase, User, Building2, BadgeCheck, Mail, Phone, Hash } from "lucide-react";

function Teachers() {
  const { teachers } = React.useContext(FirebaseContext)!;

  async function deleteRecord(uid: string) {
    if (!window.confirm("Confirm dismissal of this faculty member. This action removes all access and registry entries.")) return;
    try {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-text-muted text-sm font-medium opacity-80 pl-1">
            Active instructional staff: <span className="text-primary font-bold">{teachers.length}</span>
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-background-secondary/50 border-b border-border/20">
                <th className="py-5 px-6 font-black text-[10px] uppercase tracking-widest text-text-primary/40">Faculty Member</th>
                <th className="py-5 px-6 font-black text-[10px] uppercase tracking-widest text-text-primary/40">Contract ID</th>
                <th className="py-5 px-6 font-black text-[10px] uppercase tracking-widest text-text-primary/40">Designation</th>
                <th className="py-5 px-6 font-black text-[10px] uppercase tracking-widest text-text-primary/40">Communication</th>
                ...
                >
                  <td className="py-6 px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary transition-transform group-hover:scale-110">
                        <User size={18} />
                      </div>
                      <div className="flex flex-col">
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-text-primary leading-tight group-hover:text-primary transition-colors">{teacher.name}</span>
                          <BadgeCheck size={14} className="text-success opacity-80" />
                        </div>
                        <span className="text-[10px] font-black text-text-muted uppercase tracking-tighter mt-0.5">Verified Instructor</span>
                      </div>
                    </div>
                  </td>
                  <td className="py-6 px-6">
                  ...
                  <td className="py-6 px-6">
                    <div className="flex flex-col gap-1">
                      <div className="text-text-primary/80 text-sm font-bold flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1.5 text-text-muted text-[10px] font-bold uppercase tracking-widest">
                        <Building2 size={10} className="text-primary/30" />
                        {teacher.department}
                      </div>
                    </div>
                  </td>
                  <td className="py-6 px-6">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <div className="flex items-center gap-2 text-[10px] font-bold text-text-muted/60">
                        ...
                        {teacher.phone || "No contact record"}
                      </div>
                    </div>
                  </td>
                  <td className="py-6 px-6 text-right">
                    <div className="flex items-center justify-end gap-2 text-right">
                      ...
                      <button
                        onClick={() => teacher.uid && deleteRecord(teacher.uid)}
